  ...
  if (searchInput && searchResults) {
    ...
    searchInput.addEventListener('input', function() {
      ...
      const results = searchIndex.filter(item => {
        const cleanTitle = item.title.replace(/[^w一-龥s]/g, '').toLowerCase();
        const cleanContent = item.content.replace(/[^w一-龥s]/g, '').toLowerCase();
        const cleanTags = item.tags.map(tag => tag.replace(/[^w一-龥s]/g, '').toLowerCase());
